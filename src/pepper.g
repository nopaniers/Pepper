...
// definitions
  funcdef : decorators? name parameters EQUAL suite ;
  classdef : classname parameters? EQUAL suite ;

// compound flow statements
  while_stmt : WHILE test suite else_stmt? ;
  with_stmt : WITH test (AS expr)? (COMMA test (AS expr)?)@* suite;    // Too permissive
  if_stmt : IF test suite (ELIF test suite)* else_stmt? ;
  for_stmt : FOR exprlist IN testlist suite else_stmt? ;
  try_stmt : TRY suite (except_stmt@+ else_stmt?  finally_stmt? | finally_stmt );
  except_stmt : (CATCH (test ((AS|COMMA) test)?)? suite);
...
//    | atom molecule_star
  ...
  subscript : DOT DOT DOT
    | test
    | test? TO test? // (sliceop|test sliceop?|)
    ;
  // sliceop : TO test?  ;
...
vararg : fpdef (EQUAL test)? COLON classname ;
...
IMAG_NUMBER: '(\d+[jJ]|((\d+\.\d+)([eE][-+]?\d+)?|\d+[eE][-+]?\d+)[jJ])';
...
CLASSNAME: '[A-Z][a-zA-Z_0-9]*(?!r?"|r?\')';
NAME: '[a-z_][a-zA-Z_0-9]*(?!r?"|r?\')'  //"// Match names and not strings (r"...")
    (%unless
        IMPORT: 'import';
        FROM: 'from';
        GLOBAL: 'global';
